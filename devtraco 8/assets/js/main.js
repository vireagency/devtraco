...
(function () {
  ...
  const heroTyped   = heroRotator && heroRotator.querySelector('.hero-typed');

  if (heroRotator && heroTyped) {
    // Words live on the element as a JSON array in data-words for easy editing
    let words;
    try {
      words = JSON.parse(heroRotator.dataset.words || '[]');
    } catch (e) {
      words = ['Trust', 'Excellence', 'Fine Living'];
    }
    ...
    const ERASE_SPEED = 42;    // ms per character erased (slightly faster than typing)
    const HOLD_FULL   = 1600;  // pause when word is fully typed
    const HOLD_EMPTY  = 320;   // pause before next word begins
    const START_DELAY = 600;   // wait for page paint before starting

    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    if (reduceMotion) {
      // Static fallback — show the first word with no animation
      ...
      const tick = () => {
        ...
        if (!erasing) {
          charIdx++;
          heroTyped.textContent = word.slice(0, charIdx);
